---
title: Clientes potenciales
feature: SOAP
description: Información general sobre las operaciones de SOAP para posibles clientes, incluidos getLead, getMultipleLeads, syncLead, syncMultipleLeads y mergeLeads, para recuperar, sincronizar y combinar registros.
exl-id: 412fa16c-bd6e-4344-83ed-33dd5381fda4
TQID: https://experienceleague.adobe.com/d4JGxzJiiG4Ogcy65JpU%2D%2D%2DUhcNZrAlkaED5mstY4QU
product_v2:
  - id: b27e5950-9033-45ac-9f86-eb22e567f615
role_v2:
  - id: c66ffd68-0f65-42bb-aa23-b4020f12e0bd
source-git-commit: 4d7272e852691d956925ff12924d27522e74d8a7
workflow-type: tm+mt
source-wordcount: 37
ht-degree: 5%

---

# Clientes potenciales

Las siguientes llamadas de SOAP se pueden utilizar para interactuar con posibles clientes.

- [getLead](getlead.md)
- [getMultipleLeads](getmultipleleads.md)
- [syncLead](synclead.md)
- [syncMultipleLeads](syncmultipleleads.md)
- [mergeLeads](mergeleads.md)
