...
feature: SOAP
description: SOAP syncCustomObjectsLlamadas a la función de
...
Acepta una matriz de objetos personalizados para crear o actualizar, hasta un máximo de 100 por llamada. Devuelve el resultado (estado) de la operación (CREATED, UPDATED, FAILED, UNCHANGED, SKIPPED) y los objetos personalizados procesados. La API se llama en uno de los tres modos de funcionamiento:

1. INSERT - sólo insertar objetos nuevos, omitir objetos existentes
1. UPDATE: solo actualizar objetos existentes, omitir objetos nuevos
1. UPSERT - insertar objetos nuevos y actualizar objetos existentes

En una sola llamada de API, algunas actualizaciones pueden realizarse correctamente y otras pueden fallar. Se devuelve un mensaje de error por cada error.

Para los objetos personalizados aprovisionados con la nueva interfaz de usuario de objetos personalizados, solo los campos designados como `dedupe` campos en la interfaz de usuario se pueden pasar como atributos en `CustomObjKeyList`. Los campos de vínculo que no son `dedupe` campos deben pasarse como atributo en `customObjAttributeList`.
...
| customObjectList->customObject->customObjKeyList->attribute | Obligatorio | El atributo es un par clave/valor que se utiliza para identificar el objeto personalizado. Puede tener varios atributos en customObjKeyList |
| customObjectList->customObject->customObjAttributeList->attribute | Obligatorio | Pares clave/valor, donde el nombre es el nombre del campo y el valor es el valor que desea insertar en customObject |
