...
También puede incluir los parámetros de consulta opcionales `fields`, para devolver campos de oportunidad adicionales, `nextPageToken`, para conjuntos de paginación mayores que el tamaño del lote, `batchSize`, que toma el valor predeterminado de y tiene un máximo de 300.  Al solicitar una lista de `fields`Sin embargo, si se solicita un campo en particular, pero no se devuelve, el valor se entiende como nulo.
...
La API responderá con la variable `marketoGUID` para cada registro, así como un `status` , que indica el éxito o el fracaso individual de cada registro, y una `seq` campo que se utiliza para correlacionar los registros enviados con el orden de la respuesta.  El número del campo es el índice del registro enviado en la solicitud.
...
El [Obtener campo de oportunidad por nombre](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Opportunities/operation/getOpportunityFieldByNameUsingGET) el extremo recupera los metadatos de un único campo en el objeto de empresa.  El requerido `fieldApiName` El parámetro path especifica el nombre de API del campo.  La respuesta es como el extremo de describir oportunidad, pero contiene metadatos adicionales como `isCustom` atributo que indica si el campo es un campo personalizado.
...
Puede eliminar oportunidades desduplicando campos o campos de ID. Especifique mediante la variable `deleteBy` parámetro con un valor de deduplicationFields o idField. Si no se especifica, el valor predeterminado es deduplicarCampos. El cuerpo de la solicitud contiene un `input` una amplia gama de oportunidades de eliminación. Se permiten un máximo de 300 oportunidades por llamada.
